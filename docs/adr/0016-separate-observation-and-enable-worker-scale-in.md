...
2026-09-23 후속 기반 작업에서 단일 운영 클라이언트의 `auto`/`fixed` 모드와
중단 기록 기반 제어권 복원을 추가했다. 현재 절차는
[worker 제어권 운영](../worker-control.md)을 따른다. 이 ADR의 위 문단은 최초
자동 증감 수용 당시의 범위를 기록한다.
