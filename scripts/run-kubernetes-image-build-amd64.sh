...
[[ "$(uname -m)" == "x86_64" ]] || {
  echo "AMD64 image builder must run natively on x86_64" >&2
  exit 1
}
[[ -c /dev/kvm ]] || {
...
cd "${capi_root}"
# v0.1.55 retains generated legacy JSON as .tmpl. The contents are valid
# Packer JSON; materialize the exact pinned template before initialization.
cp packer/qemu/packer.json.tmpl packer/qemu/packer.json
make deps-qemu
# The generator prints the one-time Packer SSH password, so keep it out of the
# persisted project build log.
make set-ssh-password >/dev/null

output_dir="${capi_root}/output/ubuntu-2204-kube-${KUBERNETES_VERSION}"
...
packer_args+=("-var-file=${capi_root}/packer/qemu/qemu-ubuntu-2204.json")
packer_args+=("-var-file=${BUILD_INPUT_DIR}/image-builder-variables-amd64.json")
packer_args+=("-var=ansible_user_vars=provider=openstack")

"${capi_root}/.local/bin/packer" build "${packer_args[@]}" \
  packer/qemu/packer.json

built_image="${output_dir}/ubuntu-2204-kube-${KUBERNETES_VERSION}"
[[ -s "${built_image}" ]] || {
...
printf '%s\n' \
  ...
  "architecture=x86_64" \
  "firmware=bios" \
  "base_os=ubuntu-22.04.5" > "${result_image}.build.txt"

echo "Kubernetes AMD64 image built: ${result_image}"
